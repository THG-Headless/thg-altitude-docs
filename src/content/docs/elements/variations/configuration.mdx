...
- **`'blocking'` (default)**:  Renders variations immediately with full functionality. The component
  displays a presentational layer using the provided product data, then hydrates interactive
  elements. This reduces Cumulative Layout Shift (CLS) and provides immediate user
  interaction.
- **`'async'`**: Shows a loading state initially and defers variation hydration until manually triggered.

When using `'blocking'` mode, product data **must** be sent to the component within the [options](/packages/elements/#options) prop. This will be used to display a presentational layer before interactive elements are hydrated, which reduces Cumulative Layout Shift (CLS). 
