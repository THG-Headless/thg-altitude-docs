---
title: Version 2.4 Edge Configuration
---
...
| healthcheck                  | No       | [Health Check Configuration](#health-checks) |                                       | Configure health checks for function routes to enable automatic failover when workers become unhealthy. **Only applicable to function routes.**                                      |
...
Health checks allow you to monitor the health of your Cloudflare Worker functions. When a health check fails, traffic is automatically routed to Altitude Anywhere to ensure high availability.

**Note:** Health checks are only applicable to function routes. They are not supported for static or proxy routes.

| Key              | Required | Default | Type          | Examples                                   | Description                                                                                      |
| ---------------- | -------- | ------- | ------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| path             | Yes      |         | String        | /altitude/health                           | The path for the health check endpoint on your worker                                            |
| intervalMillis   | No       | 15000   | Integer       | 30000                                      | How often to perform the health check in milliseconds                                            |
| timeoutMillis    | No       | 5000    | Integer       | 10000                                      | How long to wait for the health check response in milliseconds                                   |
| threshold        | No       | 3       | Integer       | 2                                          | How many checks within the window must be successful for the backend to be considered healthy    |
| window           | No       | 5       | Integer       | 3                                          | Number of most recent health check queries to evaluate for thresholding                          |
| initial          | No       | 4       | Integer       | 3                                          | Number of health checks to initially consider as successful before real checks begin             |
| expectedResponse | No       | 200     | Integer       | 204                                        | The expected HTTP status code for a healthy response                                             |
| method           | No       | HEAD    | String        | GET                                        | HTTP method to use for the health check request. Allowed values: `HEAD`, `GET`                   |
| headers          | No       |         | Array[String] | `["X-Altitude-Instance: www.example.com"]` | Custom headers to include in the health check request, in the format `Header-Name: Header-Value` |
...
For more details on health checks and failover behaviour, see [Health Checks](/docs/platform/v2.4.0/edge/health-checks).

### Adding a New Route

_When adding a new route to your altitude yaml ensure it's always added at the bottom of the routes list._
