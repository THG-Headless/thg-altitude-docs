...
For applications using a bundler, you can initialise Insights programmatically:

```javascript
import { insights } from '@thg-altitude/insights';
...
|----------|----------|-------------|
| `channel` | Yes | Your brand or channel identifier |
| `subsite` | Yes | Your locale or subsite identifier |
| `storefront` | Yes | Your storefront category |
| `pageType` | No | Page type for analytics context |

### `cleanup()`

Stop tracking and clean up all event listeners. Useful for single-page applications when navigating away or reinitialising:

```javascript
import { cleanup } from '@thg-altitude/insights';

// Stop all tracking
cleanup();

// Re-initialise with new config if needed
insights({ channel: 'newchannel', subsite: 'en', storefront: 'nutrition' });
```
...
- **[Payload Schema](./payload-schema)** - Understand the data structure
