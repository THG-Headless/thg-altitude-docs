...
Add www.example.co.uk and www.example.fr to your domains key:

```js
// config/site.js
export default {
  domains: ["www.example.co.uk", "www.example.fr"],
  commerce: {
...
The `i18n.locales<Object>.domain` key must correspond to the domain you added in the `domains` array in step 1. 
...
At request time, astro-integration will use the [X-Forwarded-Host](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/X-Forwarded-Host) header (in production) to decide which config to read.
