...
Commerce API is the GraphQL API powering THG's enterprise e-commerce platform.
...
Commerce API is designed to be used by both API clients (e.g. mobile apps, scripts) and by browser-based web clients.
...
These flags are used by Commerce API to signal information about the server's current status, and the status of the user's session.  Currently, the following flags are supported:
