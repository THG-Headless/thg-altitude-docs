---
title: "Friendly Captcha Examples"
order: 4
...
Friendly Captcha provides privacy-focused, GDPR-compliant bot protection that automatically solves puzzles without user interaction in most cases. It uses the Friendly Captcha SDK v0.1.31 and supports three different start modes.

## Auto Mode (Default)

Auto mode starts the puzzle automatically when the widget loads, providing the smoothest user experience.
...
Focus mode starts the puzzle when the widget gains focus, providing user control over when verification begins.
...
await friendlyCaptcha.render('captcha-container', 'login-form', {
    startMode: 'focus', // Widget starts when user focuses on it
    onToken: (captchaToken) => {
        console.log('Friendly Captcha solved:', captchaToken);
    ...
    onExpired: () => {
        showMessage('Security verification expired. Please focus on the widget again.');
    }
});
```
...
function LoginForm() {
    ...
    useEffect(() => {
        ...
        if (captchaRef.current) {
            captcha.current.render(captchaRef.current, 'login-form', {
                ...
                onExpired: () => {
                    setCaptchaToken(null);
                    setError('Security verification expired. Please focus on the widget again.');
                }
            });
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!captchaToken) {
            setError('Please complete security verification by focusing on the widget.');
            return;
        }
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div ref={captchaRef} id="captcha-container"></div>
            <p className="captcha-hint">Click or tap the widget above to start verification</p>
...
### Best Practices
- **Auto mode**: Best for forms where verification can happen immediately
- **Focus mode**: Good for giving users control over when verification starts
- **None mode**: Ideal for invisible-like behavior, triggered on form submission
