...
Friendly Captcha can work in an invisible-like mode using `startMode: 'none'` which requires manual execution:
...
await friendlyCaptcha.render("captcha-container", "form-protection", {
  startMode: "none", // Requires manual execution
  onToken: (token) => {
...
function ContactForm() {
  ...
  useEffect(() => {
    ...
    friendlyCaptcha.render(containerId, captchaId, {
      startMode: "none", // Manual execution
      onToken: (captchaToken) => {
        submitForm(captchaToken);
      },
      onError: () => {
