...
const captcha = new Captcha({
    provider: 'hcaptcha', // 'recaptcha' | 'hcaptcha' | 'turnstile' | 'friendlyCaptcha'
...
Friendly Captcha offers a privacy-focused solution that automatically solves puzzles without user interaction in most cases:
...
// Auto start mode (default) - puzzle starts automatically when widget loads
await friendlyCaptcha.render('captcha-container', 'signup-form', {
...
// Focus start mode - puzzle starts when widget gains focus
await friendlyCaptcha.render('captcha-container-2', 'login-form', {
    startMode: 'focus', // Widget starts when user focuses on it
    onToken: (token) => setLoginToken(token)
});

// None start mode - requires programmatic execution
await friendlyCaptcha.render('captcha-container-3', 'checkout-form', {
    startMode: 'none', // Widget waits for execute() call
    onToken: (token) => setCheckoutToken(token)
});

// Later, trigger the puzzle manually by calling widget.start()
