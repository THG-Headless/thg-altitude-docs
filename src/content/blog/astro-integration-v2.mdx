...
We've completely separated tenancy and internationalization concerns, giving you much more flexibility:
...
- **Multi-tenant + i18n**: Full flexibility to mix tenants with and without internationalization
...
- **Cookie routing** has been removed. This feature was deprecated because it was only used by the [account](https://github.com/THG-Headless/account) repository, which has since migrated to resolving locales directly from the `Host` request header in production, and the `x-altitude-instance` header in local development.
- Several global context keys like `preferredLocale`, and `localeCookie` are no longer bound to `context.altitude`.
