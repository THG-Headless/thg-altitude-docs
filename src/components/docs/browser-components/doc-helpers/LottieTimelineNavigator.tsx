...
const LottieTimelineNavigator: React.FC<LottieTimelineNavigatorProps> = ({
  ...
  // If timestamps array is empty, display an error message
  if (timestamps.length === 0) {
    console.error(
      "No timestamps provided or timestamps is not an array",
      rawTimestamps
    );
    return <div className="error">Error: No timeline data available</div>;
  }
  ...
  const renderProgressBar = () => {
    // Make sure we have valid timestamps before rendering
